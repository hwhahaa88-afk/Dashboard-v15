...
// استبدال دالة vmove بأمان دون إفساد بقية الأقواس
const oldVmoveRegex = /\{\s*name:\s*'vmove'[\s\S]*?execute:\s*async\s*\(ctx\)\s*=>\s*\{[\s\S]*?\}\s*\},/;
...
const newVmove = `{
    ...
    execute: async (ctx) => {
      ...
      if (!channel) {
        return ctx.reply(r('❌', 'You must specify a voice channel or be in one yourself.'));
      }
...
if (oldVmoveRegex.test(code)) {
  code = code.replace(oldVmoveRegex, newVmove);
  fs.writeFileSync('commands.js', code);
  console.log('✅ Updated vmove command successfully');
} else {
  console.log('⚠️ Could not match vmove command pattern automatically');
}
